...
Grafika koncepcyjna bêd¹ca podstaw¹ realizacji czêœci praktycznej pracy zosta³a zaczerpniêta z jednej z edycji \textit{Monthly Community Noob Challenge}. W maju 2015 roku postawiono przed u¿ytkownikami forum wyzwanie stworzenia wnêtrza œwi¹tyni. Obraz, na którym wzorowali siê uczestnicy by³ równie¿ punktem wyjœcia dla realizacji tej pracy:
	\begin{figure}[!ht]
		...
		\caption{\textit{Monthly Community Noob Challenge 30} - grafika koncepcyjna (Ÿród³o: \cite{NoobChallengeMay2015})}
	\end{figure}
...
Ostatnia faza polega³a na dopracowywaniu pokazowej sceny w taki sposób, aby sta³a siê ona jak najatrakcyjniejsza dla potencjalnego ogl¹daj¹cego. Najwa¿niejszymi zabiegami, które zosta³y tutaj zastosowane by³y:
\begin{itemize}
	\item \textbf{Dopasowanie parametrów materia³ów wykorzystanych w projekcie} \newline
	Odpowiednie sparametryzowanie materia³ów pozwala na uzyskanie lepszych efektów wizualnych, w zale¿noœci od zamiarów twórcy. \textit{Unreal Engine 4} pozwala chocia¿by na zmianie szorstkoœci materia³u, intensywnoœci emisji, czy te¿ przeŸroczystoœci. Ich odpowiednie ustawienie pozwala na osi¹gniêcie wiêkszego realizmu. Odnalezienie odpowiedniego kompromisu pomiêdzy poziomem komplikacji materia³ów, a efektem wizualnym jest wiêc kluczowe dla jakoœci pakietu. \newline
	
	W zaprezentowanej pracy skupiono siê g³ównie na odpowiednim zaznaczeniu rodzaju materia³u, którego u¿ywa dana powierzchnia (a w szczególnoœci na parametrze metalness, który odpowiada za jego ''metalicznoœæ'').\newline
	\item \textbf{Dodanie efektów graficznych} \newline
	Efekty graficzne pozwalaj¹ na dodatkowe wzbogacenie sceny. Dobrym przyk³adem jest tu wykorzystanie efektów cz¹steczkowych (ang. particle effects) do ''o¿ywienia'' sceny. Przyk³adowo - poziom wykonany podczas realizacji projektu zawiera w sobie p³on¹ce znicze; efekt ten zosta³ uzyskany dziêki wykorzystaniu wy¿ej wymienionej techniki.\newline
	
	Innym efektem, który warto tu wspomnieæ jest postprocessing sceny. Efekty, takie jak rozmycie (ang. blur), czy okluzja otoczenia (ang. ambient oclussion) pozwalaj¹ na poprawienie finalnego efektu. Nale¿y jednak pamiêtaæ, ¿e ich wykorzystywanie czasami znacznie zwiêksza zapotrzebowanie na zasoby komputera podczas renderowania sceny.\newline
	\item \textbf{Dodanie oœwietlenia} \newline
	We wczeœniejszych rozdzia³ach wspomniane by³o kilkukrotnie o tym, jak wa¿ne jest odpowiednie oœwietlenie poziomu. Pozwala ono na oddanie odpowiedniego klimatu dla sceny oraz na wyeksponowanie lub ukrycie pewnych jego fragmentów. \newline
	
	W pracy wykorzystano ¿ó³tawo-pomarañczowe oœwietlenie, które dobrze oddaje klimat pomieszczeñ oœwietlanych przy pomocy otwartego ognia. Dodatkowo - zastosowanie niewielkiej intensywnoœci jeszcze bardziej wzmacnia ten efekt. Porównanie poziomu z ustawionym w scenie i wypieczonym przez silnik oœwietleniem mo¿na zobaczyæ na rysunku 4.7.
	\begin{figure}[!ht]
		\centering
		\includegraphics[scale = 0.30]{figures/lighting_comparsion}
		\caption{Porównanie sceny przed i po ustawieniu oœwietlenia (Ÿród³o: opracowanie w³asne)}
	\end{figure}
\end{itemize}
Kieruj¹c siê wytycznymi wyznaczonymi na wczeœniejszych etapach pracy powsta³ pakiet 40 trójwymiarowych modeli oteksturowanych przy pomocy 3 atlasów w rozdzielczoœci 4096 na 4096 pikseli (po jednym atlasie dla map diffuse, normal i roughness). Zrzut ekranu z programu \textit{3ds Max 2016} prezentuj¹cy wszystkie wymodelowane siatki wielok¹towe (wraz z teksturami na nich) mo¿na zobaczyæ na rysunku 4.8.
\begin{figure}[!ht]
	\centering
	\includegraphics[scale = 0.35]{figures/My_assets_pack}
	\caption{Modele stworzone na potrzeby projektu (Ÿród³o: opracowanie w³asne)}
\end{figure}

Z ukazanych wy¿ej assetów powsta³a scena wzorowana na wybranej wczeœniej grafice koncepcyjnej. Porównanie efektu uzyskanego w trakcie realizacji pracy z punktem wyjœciowym zosta³o ukazane na rysunku 4.9.
\begin{figure}[!ht]
	\centering
	\includegraphics[scale = 0.25]{figures/Comparsion_with_concept}
	\caption{Porównanie grafiki koncepcyjnej ze scen¹ utworzon¹ podczas realizacji czêœci praktycznej pracy (Ÿród³o: opracowanie w³asne)}
\end{figure}

Przestrzeñ utworzon¹ w tej pracy dodatkowo ukazuj¹ ilustracje 4.10 oraz 4.11. Natomiast na rysunku 4.12 zawarto przyk³ad detalu - lampionu z efektem cz¹steczkowym p³omienia.
\begin{figure}[!ht]
	\centering
	\includegraphics[scale = 0.20]{figures/space}
	\caption{Scena z perspektywy wejœcia niewidocznego na grafice koncepcyjnej (Ÿród³o: opracowanie w³asne)}
\end{figure}
\begin{figure}[!ht]
	\centering
	\includegraphics[scale = 0.20]{figures/persp_1}
	\caption{Scena z perspektywy ukazuj¹cej niewidoczn¹ na grafice koncepcyjnej czêœæ œwi¹tyni (Ÿród³o: opracowanie w³asne)}
\end{figure}
\begin{figure}[!ht]
	\centering
	\includegraphics[scale = 0.20]{figures/detail}
	\caption{P³on¹cy lampion widoczny z bliskiej odleg³oœci (Ÿród³o: opracowanie w³asne)}
\end{figure}